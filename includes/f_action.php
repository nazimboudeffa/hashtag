...
$checkfollow_sql = "SELECT * FROM follow WHERE uf_one=:s_id AND uf_two=:user_id";
    $checkfollow = $con->prepare($checkfollow_sql);
...
if ($fchecknum > 0) {
    ...
    $unfollow = $con->prepare($unfollow_sql);
    ...
    $UpdateFollowersSql = "UPDATE users SET followers=:followers_num WHERE id=:user_id";
    $UpdateFollowers = $con->prepare($UpdateFollowersSql);
