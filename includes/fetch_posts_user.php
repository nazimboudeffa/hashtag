...
$checkIfollowedHim = $con->prepare($checkIfollowedHim_sql);
...
$vpssql = "SELECT * FROM posts WHERE author_id=:row_id AND p_privacy != :pr_onlyme  ORDER BY post_time DESC LIMIT :plimit,10";
$view_posts = $con->prepare($vpssql);
...
$vpssql = "SELECT * FROM posts WHERE author_id=:row_id AND p_privacy != :pr_onlyme AND p_privacy != :pr_followers ORDER BY post_time DESC LIMIT :plimit,10";
$view_posts = $con->prepare($vpssql);
$view_posts->bindValue(':row_id', $row_id, PDO::PARAM_INT);
$view_posts->bindValue(':pr_onlyme', $pr_onlyme, PDO::PARAM_INT);
...
$vpssql = "SELECT * FROM posts WHERE author_id=:row_id ORDER BY post_time DESC LIMIT :plimit,10";
$view_posts = $con->prepare($vpssql);
$view_posts->bindValue(':row_id', $row_id, PDO::PARAM_INT);
$view_posts->bindValue(':plimit', (int)trim($plimit), PDO::PARAM_INT);
$view_posts->execute();
