...
$getTriendsPagesSql = "SELECT * FROM users ORDER BY followers DESC LIMIT 6";
$getTriendsPages = $con->prepare($getTriendsPagesSql);
...
$emptypost = "";
$public = "0";
$getTriendsPostsSql = "SELECT * FROM posts WHERE post_content!= :emptypost AND p_privacy = :public ORDER BY p_likes DESC LIMIT 6";
$getTriendsPosts = $con->prepare($getTriendsPostsSql);
...
while ($fetch = $getTriendsPosts->fetch(PDO::FETCH_ASSOC)) {
    ...
    $aop_trend_sql = "SELECT * FROM users WHERE id=:authorOfPost";
    $aop_trend = $con->prepare($aop_trend_sql);
